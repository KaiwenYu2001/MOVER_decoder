...
def Execute(patient_dir: str, output_dir: str, debug=False):
    '''
    ...
    for IP_type in [True, False]:
        flist = BT.Find_XML_in_DIR(input_dir, "sampleID", "2025-01-10", debug=False, IP=IP_type)
    ...
    Wave = BT.load_xml_into_wave_assembly(flist, debug=False)
    if debug:
    ...
    for waveType in waveInfoLib:
        if waveInfoLib[waveType] != 0:
            ...
            output_filename = os.path.join(output_dir, f"{os.path.split(patient_dir)[1]}.csv")
...
if __name__ == '__main__':
    patient_id = "14311ee2b6fae5c4"
    input_dir = "/Users/kevinyu/Desktop/test/14311ee2b6fae5c4"
    output_dir = "/Users/kevinyu/Desktop/result"
    Execute(input_dir, output_dir, True)